...
import type { DayPlannerData, EO } from '@/lib/types';
...
interface DayPlannerDropzoneProps {
    planner: DayPlannerData;
    isActive: boolean;
    onActivate: () => void;
    onRemoveEo: (plannerId: string, eoIndex: number) => void;
}

export function DayPlannerDropzone({ planner, isActive, onActivate, onRemoveEo }: DayPlannerDropzoneProps) {
    ...
    return (
        <Card className={cn(isActive && "ring-2 ring-primary ring-offset-2 bg-primary/10")}>
                        ...
                        {planner.eos.map((eo, index) => (
                            <div key={`${eo.id}-${index}`} className="relative p-2 rounded-md border bg-background/50 text-sm">
                                ...
                                <Button 
                                    ...
                                    onClick={(e) => { e.stopPropagation(); onRemoveEo(planner.id, index); }}
                                >
                                    <X className="h-3 w-3" />
                                </Button>
                            </div>
                        ))}
                        {planner.eos.length === 0 && <p className="col-span-full self-center text-muted-foreground text-center py-10">This planner is empty. Activate it by clicking here, then add EOs from the left panel.</p>}
                    </div>
                </CardContent>
            </div>
        </Card>
